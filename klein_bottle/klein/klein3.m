...
g = @(theta, phi) min(scale - mod(abs(theta-theta0),scale), mod(abs(theta-theta0),scale)) + min(scale*height - mod(abs(phi-phi0),scale*height), mod(abs(phi-phi0),scale*height));

x = g(thetas, phis);
y = g(thetas + scale/2,-phis);
