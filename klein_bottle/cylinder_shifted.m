...
obsfn = @(theta, z) min(d_cyl(theta,z), d_cyl(mod(theta+.5, 1), sign(z)*inverse_fatness + sign(z)*abs(sign(z)*inverse_fatness-z)));

%Fill out a trajectory on the cylinder
NTotal = 10000;
NPeriods = 100;
zs = linspace(-inverse_fatness, inverse_fatness , NTotal);
